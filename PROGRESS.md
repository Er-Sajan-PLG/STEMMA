...
Increments 1–3 landed 2026-09-23 — projection mechanics; **PID-agnostic per the R5 Amendment**: entities project as `stemma:`-scheme opaque URIs (the scheme the amendment binds), no external identifier base claimed; the PID/resolution decision stays gated to the publication step and will be re-validated against then-current data (identifier infrastructure drifts — the purl.org lesson from the R5 brief).
...
- [x] Increment 3 landed 2026-09-23: (a) XOR now DECLARATIVE — `sh:xone` in stemma-shapes.ttl (core SHACL property-pair logical constraint, no SPARQL needed), enforced in the subset runner with xone-vs-minCount separation + path-count parity tests, and redundantly in validate_jsonld.py (defense in depth). (b) `scripts/sign_release_bundle.py` — owner-key gpg signing mechanics (bundle checksum + detached armor signature; never fabricates keys; exit 2 with setup guidance when no gpg/key). (c) Content batch seeded as draft proposals (R7 feedstock): `stemma:phys.second` (Q11574) + `stemma:phys.kilogram` (Q11570) at metre-bar definitions — completes the SI-unit triangle for the 3 seeded base quantities (metre↔length existed). Awaiting owner review (human:curator.001) — review commands same as the R4 queue. R7 calibration counter: 11/200 labeled decisions after those land.
- [ ] R6 remaining: owner publication decision (publication_gate.py BLOCKED by design until docs/decisions/r6-identifier-base.md, human:* signed, candidate set w3id|datacite-doi|ark-n2t|stemma-urn-only, fresh-data re-verified)
