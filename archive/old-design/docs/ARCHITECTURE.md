...
## 7. Reference Vertical Slice: Physics-Core (ADR-0040)

To prove the architecture, we implement a depth-first vertical slice:

```
content/physics/{mechanics,electricity-magnetism,thermal-physics}/ ~70 entities minimal profile
connections/ ~150 connections using only 7 adopted relations (ADR-0042)
sources/ 5 canonical textbook records
→ validate.py → exports/knowledge.json v2.1.0 → explorer DAG + adapter search
```

This slice exercises all layer invariants with real science: identity, domain/range, cycles over dependency family, duplicate signatures, evidence, review. Zero `related_to` in physics/. If physics-core canonicalizes, the engine is proven; other domains reuse the pattern.

See `docs/PHYSICS-FIRST-IMPLEMENTATION-PLAN.md`.
...
connections-only relationship truth), ADR-0029 (refoundation baseline), and
ADR-0040/0041/0042 (physics-first minimal strategy, minimal profile, minimal relation set).
