...
## 8. Physics-Core Profile (ADR-0040/0041/0042)

**Status:** NOW — depth-first vertical slice.

### 8.1 Minimal Entity Profile for Physics

Required: `id, type, name, domain, status, definition, provenance`

Allowed: `symbol, equation, unit, examples (max1), external_ids, historical, aliases, version, updated_at`

Forbidden for physics/: `learning_objectives, real_world_applications, key_experiments, common_misconceptions` — policy enforced by `physics_core_profile_check.py`. See ADR-0041. No schema break in v0.1; formal removal in v2.0.

Future (ADR-0024): `math: {latex, symbol_bindings, variants}, dimensions: {L,M,T,I,Θ,N,J}, unit_ref, constant`

### 8.2 Physics Subdomain Taxonomy (v0.1 NOW)

- `mechanics` (priority 1)
- `electricity-magnetism` (priority 2)
- `thermal-physics` (priority 3)
- `measurement-units` (for unit entities)
- DEFER: `waves-optics`, `atomic-nuclear`

### 8.3 Physics Modeling Patterns

**Quantity:** definition + symbol + unit (string) + dimensions (future) + external_ids wd/qudt + appears_in_law / mathematically_required_by

**Unit:** symbol + dimensions + system SI + external_ids qudt/ucum/wd + conversion to coherent SI

**Law:** definition + equation display + mathematically_requires -> quantities (3-4) + applies_to -> model/phenomenon + historical

**Equation:** latex + derived_from -> law

**Concept/Model:** definition + part_of / approximates

### 8.4 Minimal Relation Set (ADR-0042)

Physics-core may use only 7 adopted relations: `mathematically_requires`, `derived_from`, `appears_in_law`, `applies_to`, `generalizes`/`special_case_of`, `part_of`, `approximates`. Zero `related_to` in physics/. See RELATIONSHIP-SPECIFICATION.md §2.1.1.

## 9. Out of the domain model (consumer responsibilities)
...
structure to build these on. For physics-core, `learning_objectives` etc. are explicitly consumer-side, not canonical (ADR-0041).
