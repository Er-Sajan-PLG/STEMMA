...
**Status:** Authoritative evidence-based ledger (baseline 3.0.0 + physics-first amendment 2026-09-21).
Claims here are backed by the verification chain, not by older documents.
Current focus: `docs/PHYSICS-FIRST-IMPLEMENTATION-PLAN.md` (ADR-0040/0041/0042).
...
## 2. Partially implemented (honest gaps) — pre-refoundation baseline
...
| Math layer | Display strings only (`equation`/`symbol`/`unit`); 1 `unit` entity; no symbol→quantity bindings | ADR-0024 proposed — awaits human gate G-C, now linked to physics-first R2 |
...
## 2.1 Physics-Core v0.1 Track (NOW per ADR-0040/0041/0042)

| Area | Target | Current | Gap |
|---|---|---|---|
| Physics entities (mechanics, EM, thermo) | 70 | 0 | Seed from Halliday Resnick + Griffiths + Schroeder |
| Physics connections (7 relations only) | 150 | 0 | No `related_to` allowed |
| Source records | 5 | 0 | HRW 12th, Griffiths 4th, Schroeder, NIST SI, CODATA |
| Minimal profile enforcement | 100% | 0% | `physics_core_profile_check.py` + test |
| Human review | 70 entities, 150 connections canonical | 0 | Campaign via `dependency_review_campaign.py --domain physics` |
| Math layer pilot | 20 quantities + 5 laws with `math` + `dimensions` + `unit_ref` | 0 | Needs ADR-0024 decision with adjustments |
| Explorer DAG | Connected graph, not isolated dots | Empty | Requires connections |
| Adapter search | `force` closure returns mass, acceleration | Empty | Requires connections |

Evidence: `scripts/status_truth.py` filtered to domain=physics + `scripts/physics_core_profile_check.py` (new).

## 2.2 Refoundation Baseline (Current Repo State)

After ADR-0029 refoundation, canonical corpus is 0 entities / 0 connections / 0 sources (intentional empty baseline). Previous counts (224/654/3) are historical pre-refoundation. README status block now shows 0/0/0 and is CI-checked. Physics-first starts from this clean slate.
