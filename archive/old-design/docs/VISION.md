...
## Depth-First Strategy (ADR-0040)

VISION non-goal "Coverage of the whole of science" is operationalized as **physics-first, depth-first**.

All-STEM breadth makes human review impossible (604 unreviewed / 654 connections pre-refoundation). The engine is domain-agnostic; curation is not. We therefore prove the canonicalizing engine with highest-consensus, most-mathematized science first:

- **NOW (v0.1):** Physics core — mechanics, electricity-magnetism, thermal-physics + measurement-units. ~70 entities, ~150 connections, 7 adopted relations, minimal profile (ADR-0041/0042). This vertical slice validates gate → export → adapter → explorer chain.
- **LATER:** Other physics subdomains (waves-optics, atomic-nuclear), then chemistry foundations, then other STEM domains — each reusing the physics-core pattern.
- **Principle:** Depth and correctness in one domain before breadth across STEM. A connected DAG of 70 physics entities is more valuable than 500 disconnected glossary entries.

See `docs/PHYSICS-FIRST-IMPLEMENTATION-PLAN.md` and ADR-0040/0041/0042.
...
- A general-purpose ontology or reasoning system. STEMMA models the domain it
  needs, no more.
- Coverage of the whole of science at once (depth-first via physics-core, breadth later — ADR-0040). Depth and correctness over breadth.
- Attempting all STEM domains simultaneously before physics-core is canonical (>50% reviewed).
