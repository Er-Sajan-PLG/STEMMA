...
{{
  "id": "stemma:<domain>.<slug>",  // e.g. stemma:phys.doppler-effect, stemma:chem.catalysis, stemma:math.derivative
  ...
  "provenance": {{
    "ai_drafted": true,
    "source": "{getattr(blueprint, 'source_ref', 'stemma:src.unknown')}",
    "reviewer": null,
    "reviewed_at": null
  }},
  "relationships": []
...
def _deterministic_fallback(blueprint: Any, text: str, data: Dict[str, Any]) -> Dict[str, Any]:
    bp = blueprint
    source_ref = getattr(bp, 'source_ref', 'stemma:src.unknown')
    raw_title = data.get('title') or 'concept'
    slug_name = re.sub(r'[^a-z0-9]+', '-', raw_title.lower()).strip('-')

    return {
        "id": f"stemma:phys.{slug_name or 'concept'}",
        "type": "concept",
