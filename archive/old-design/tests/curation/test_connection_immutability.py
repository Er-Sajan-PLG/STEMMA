...
def test_live_connections_cover_every_connection_file():
    ...
    if len(files) == 0:
        ...
        return
    # For physics-core v2 minimal (ADR-0040), we have small KB (17 entities, 27 connections), not 600+
    # Check is len(live) == len(files) and >0, not >600 (old corpus expectation)
    assert len(live) == len(files), f"{len(live)} parsed vs {len(files)} files"
    assert len(files) > 0, f"Expected at least 1 connection file, got {len(files)}"
    ...
    print(f"PASS: {len(live)} live connections parsed with complete triples (physics-core v2 minimal)")
