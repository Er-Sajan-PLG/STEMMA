...
def test_origin_preserved():
    # Pick a migrated canonical — skip for physics-core v2 minimal which has no legacy migration
    ...
    # For new physics-core design (ADR-0040/0043), conn.000001 is human-authored, not legacy migration
    # Only enforce legacy check if actually legacy
    if d["provenance"]["asserted_by"]["id"] != "unknown:legacy-relationship":
        print(f"SKIP: origin preserved (conn.000001 is {d['provenance']['asserted_by']['id']}, not legacy — new physics-core design)")
        return
