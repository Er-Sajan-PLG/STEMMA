---
id: stemma:phys.metre
type: unit
name: Metre
domain: physics
subdomain: measurement-units
status: canonical
definition: 'The metre (symbol: m) is the base unit of length in the International
  System of Units (SI). It is scientifically defined as the length of the path travelled
  by light in a vacuum during a time interval of 1/299,792,458 of a second. Exact:
  c=299,792,458 m/s. Agreed per BIPM SI Brochure 9th ed. 2019 redefinition, fixing
  c exact.'
symbol: m
unit: metre (m)
same_dimensional_quantities:
- distance
- width
- height
- displacement
- wavelength
- radius
governed_by:
- stemma:phys.si-definitions
- stemma:phys.dimensional-analysis
provenance:
  ai_drafted: false
  writer: human:curator.001
  source_kind: standards-or-specification
  source: 'BIPM SI Brochure 9th ed. (2019) §2.3.1, p130-132: The metre, symbol m,
    is the SI unit of length. It is defined by taking the fixed numerical value of
    the speed of light in vacuum c to be 299792458 when expressed in m s-1. Exact:
    c=299,792,458 m/s. Agreed internationally.'
  link: https://www.bipm.org/en/publications/si-brochure
  original_author: BIPM
  retrieved_at: '2026-09-21'
  reviewer: human:curator.001
  reviewed_at: '2026-09-23T04:35:19.817235+00:00'
source_refs:
- stemma:src.nist-si-brochure-9th
external_ids:
  wd: Q11573
  qudt: unit-M
historical:
  stated_by: BIPM
  year: 2019
  where: SI Brochure 9th ed.
  timeline:
  - year: 1983
    by: BIPM
    event: Metre defined via c=299,792,458 m/s
  - year: 2019
    by: BIPM
    event: SI redefinition with fixed constants, c exact
---

The metre (symbol: m) is the base unit of length in the International System of Units (SI). It is scientifically defined as the length of the path travelled by light in a vacuum during a time interval of 1/299,792,458 of a second. Exact: c=299,792,458 m/s. Agreed per BIPM SI Brochure 9th ed. 2019 redefinition, fixing c exact.
