---
id: stemma:phys.force
type: quantity
name: Force
domain: physics
subdomain: mechanics
status: human_reviewed
definition: 'Force is the derived physical quantity of mechanical interaction: the
  net force on a body equals the time rate of change of its linear momentum, F = dp/dt
  (Newton, Principia Lex II); for constant mass this reduces to F = m a. Force is
  a vector quantity — it has magnitude and direction and adds by vector addition (net
  force = vector sum). SI derived unit: the newton, N = kg m s-2. Reference: Halliday,
  Resnick, Walker, Fundamentals of Physics 12th ed., Ch 5; Newton, Principia (1687),
  Lex II.'
quantity_kind: derived
tensor_character: vector
symbol: F
unit: newton (N = kg m s-2)
same_dimensional_quantities:
- weight
- tension
- friction
- normal force
- buoyant force
- thrust
- drag
governed_by:
- stemma:phys.dimensional-analysis
- stemma:phys.newtons-second-law
provenance:
  ai_drafted: true
  writer: llm:coding-agent.001
  source_kind: textbook
  source: 'Halliday, Resnick, Walker, Fundamentals of Physics 12th ed., Ch 5: net
    force ΣF = m a; Principia (1687), Lex II: mutationem motus proportionalem esse
    vi motrici impressae.'
  link: https://www.wiley.com/en-us/Fundamentals+of+Physics%2C+12th+Edition-p-9781119773511
  original_author: Halliday, Resnick, Walker
  retrieved_at: '2026-09-22'
  reviewer: human:curator.001
  reviewed_at: '2026-09-22T12:15:00.595338+00:00'
source_refs:
- stemma:src.halliday-resnick-walker-12th
- stemma:src.newton-principia-1687
external_ids:
  wd: Q14038
historical:
  stated_by: Halliday, Resnick, Walker
  year: 1687
  where: Principia, Lex II
---

Force is the derived physical quantity of mechanical interaction: the net force on a body equals the time rate of change of its linear momentum, F = dp/dt (Newton, Principia Lex II); for constant mass this reduces to F = m a. Force is a vector quantity — it has magnitude and direction and adds by vector addition (net force = vector sum). SI derived unit: the newton, N = kg m s-2. Reference: Halliday, Resnick, Walker, Fundamentals of Physics 12th ed., Ch 5; Newton, Principia (1687), Lex II.
