...
**Consumer contract:** `export_version: 2.2.0` (ADR-0050 — value-slot + delegated authority; single source: `schema/VERSION.yaml`). The export at `exports/knowledge.json` always carries the current contract; reader support for 2.1.x still works (2.2.0 is additive).
...
- `exports/knowledge.json` — main export v2.2.0 deterministic content-hash sha256, no wall clock, 1 entity now, will grow to mediocre all-domain
...
- `exports/vector_store/` — NEW: FAISS/Chroma/Qdrant local vector store — meta.json + vectors.json + ids.json, versioned via content_hash + model id, for RAG, generated via embed.py
...
- **OpenAPI:** schema/api.yaml v2.2.0 — OpenAPI 3.0.3
- **Endpoints:**
...
- **Storage:** exports/embeddings.jsonl (entity_id, model, dimensions, vector, content, content_hash) + exports/vector_store/ (FAISS meta.json + vectors.json + ids.json) — derived artifacts regenerable, versioned via content_hash + model id
