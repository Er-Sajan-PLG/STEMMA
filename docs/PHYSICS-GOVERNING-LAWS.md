...
**Status:** Authoritative for physics domain (ADR-0040/0043) with HITL, evolvable templates, model selector like DeepSeek harness (local + frontier models). This is the deterministic governing point that decides what goes where, without relying on LLM reasoning. 1 entity now (metre) via PDF primary ingestion with HITL, deterministic scales, evolvable templates, model selector like DeepSeek harness (local + frontier models). Old 74 entities archived.
...
## Scaling + Evolvable Templates + model selector like DeepSeek harness (local + frontier models) — NEW 2026-09-21)

- **Deterministic scales:** Uses schema/template-registry.yaml regex + exact SI constants c=299,792,458 m/s, h=6.62607015e-34, ΔνCs=9,192,631,770 Hz — no LLM, no cost, scales to 1000s PDFs, any domain
- **Evolvable:** Add chemistry, biology, math domains via `python3 scripts/evolvable_template.py --evolve --new-domain chemistry` without code change — templates in YAML
- **LLM fallback only when PDF missing exact SI:** If PDF text says "Length is distance" without "Exact: c=...", LLM fetches standard definition from SI Brochure/NIST using frontier model (DeepSeek R1 free, Claude 3.5 Sonnet, GPT-4o, Gemini 2.5 Pro, Llama 3.3 70B free) or custom model via OpenRouter/NVIDIA NIM, selector like DeepSeek harness (search, categories Frontier/Reasoning/Free/Custom, 25 models, custom input)
...
- **HITL before canonical:** No entity without human explicitly editing markdown file — workflow/audit/audit.jsonl must contain candidate_edited by human:* after AI draft
