# PHYSICS MINIMAL DESIGN V2 — Beginning, Clean Minimal, Scientifically Agreed Definitions

**Date:** 2026-09-21 (updated 2026-09-21 to 1 entity via HITL, deterministic scales, evolvable templates, model selector like DeepSeek harness (local + frontier models))
**Status:** BEGINNING — no legacy, 1 entity (metre) via PDF primary ingestion with HITL, deterministic scales, evolvable templates, model selector like DeepSeek harness (local + frontier models), clean 3D explorer, workflow 2 PDFs 7 candidates 3 HITL edits. Old 74 entities archived to archive/beginning-74-entities/.
**Related:** ADR-0040/0041/0042/0043, AGENT.md, PHYSICS-GOVERNING-LAWS.md, VISION.md, DOMAIN-MODEL.md
**Goal:** Minimal physics foundation where every entity has scientifically agreed definition with reference (SI Brochure, Halliday Resnick Walker, Principia) + writer/link/source_refs for triple verification + history timeline for laws + fundamental & derived quantities prominent in 3D.
...
## 2. Minimal Entity Schema — Scientifically Agreed Definition + References

### 2.1 Required Fields — Fundamental & Derived Quantities Must Be Visible

**Fundamental quantities (SI base, 7) — always present, larger in 3D:**
- `length` (L, metre), `mass` (M, kilogram), `time` (T, second), `electric-current` (I, ampere), `thermodynamic-temperature` (Θ, kelvin), `amount-substance` (N, mole), `luminous-intensity` (J, candela) — defined by fixing constants per SI 2019, agreed internationally via BIPM SI Brochure 9th ed.

**Derived quantities — from fundamentals via governing laws:**
- `area` (L², m²), `volume` (L³, m³), `speed`/`velocity` (L T⁻¹, m/s), `acceleration` (L T⁻², m/s²), `density` (M L⁻³, kg/m³), `force`/`weight` (M L T⁻², N), `pressure` (M L⁻¹ T⁻², Pa), `energy`/`work` (M L² T⁻², J), `power` (M L² T⁻³, W), `momentum` (M L T⁻¹), `angular-momentum` (M L² T⁻¹), `torque` (M L² T⁻²), `frequency` (T⁻¹, Hz), `charge` (I T, C), `voltage` (M L² T⁻³ I⁻¹, V), etc.

**All definitions must be scientifically agreed — not invented:**

```yaml
id: stemma:phys.<slug>          # stable, never reused, e.g., stemma:phys.length
type: quantity | unit | law | equation | concept | model | phenomenon
name: "Human readable"          # Title Case, e.g., Length
domain: physics
...
definition: >-                  # MANDATORY — scientifically agreed, with reference
  Length is a measure of spatial distance between two points, one of seven SI base quantities, dimension L, agreed as base per SI Brochure 9th ed. §2.3.1 and HRW 12th ed. Ch1 p4.
  # Must be verifiable, curriculum-agnostic, 1-3 sentences, states what it is + how measured + agreed formula
```
