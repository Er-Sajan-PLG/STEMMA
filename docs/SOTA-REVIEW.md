...
**Consequences to implement:** `scripts/validate.py` (report writer + `--json` +
warning collection), `scripts/integrity_anomalies.py` (advisory chain integration),
`scripts/verify_all.py` (add step, ensure exit-code policy), `reports/validation-report.json`
format tests, `docs/TESTING.md`, ADR-0033 (validation report contract).
