...
```bash
python3 scripts/validate.py  # OK 1 entities valid, export written to exports/knowledge.json v2.2.0 deterministic content-hash
...
python3 scripts/docs.py check  # OK DOCUMENTATION CONTRACT PASS — 14-category census + contract integrity + links + ADR-0029/0027 invariants + recovery registries, CI-equivalent (New 2026-09-22 documentation sync system; see docs/DOCUMENTATION-SYSTEM.md)
```

## Tests — layered

Run the full suite: `pip install -r requirements-dev.txt && python3 -m pytest tests/ -q`
(CI runs this as the `test-suite` job — the suite must stay green, no skips for
stale corpus expectations; old-design tests that encoded the archived 74/224-entity
corpus were retired to `archive/old-design/tests/` on 2026-09-22).

Gate-critical layers (also executed by `scripts/verify_all.py`):

- **registry/test_registry_coherence.py** — relation registry coherence, minimal 8 in physics core, no related_to in physics
...
- **repo/test_docs_consistency.py** — ADR-0029: required docs exist, docs/README.md indexes exactly the docs that exist, no living doc links a retired doc, VERSION.yaml keys present (live, not stubbed)
- **repo/test_independence.py** — ADR-0027/0051: no retired-doc references, no ecosystem-controller references, no retired ID namespace outside history (live, not stubbed)
...
- **Manual:** python3 scripts/embed.py --list-models — lists 11 models local free + frontier API, model selector like DeepSeek harness (local + frontier models)
...
- **Derived tests (INFO, not FAIL):** embed.py — generates embeddings.jsonl + vector_store/ FAISS meta.json + vectors.json + ids.json, deterministic same content_hash + model id → same embeddings, versioned, regenerable — verify_all.py checks existence as INFO "run embed.py" if missing, doesn't fail gate
