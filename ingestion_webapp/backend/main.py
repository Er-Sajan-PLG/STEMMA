...
async def upload_file(
    ...
    source_data = {
        "id": f"stemma:src.ingest-{file_id}",
        "type": "source",
...
async def extract_file(file_id: str, ocr_max_pages: int = Form(default=50)):
    ...
    try:
        ...
        source_candidate = ingest.build_source_candidate(extraction, source_id=f"stemma:src.ingest-{file_id}")
...
async def create_proposal(file_id: str, request: ProposalRequest):
    ...
    try:
        ...
        source_candidate = ingest.build_source_candidate(
            ...
            source_id=f"stemma:src.ingest-{file_id}"
        )
...
async def canonicalize_proposal(
    ...
    if artifact_id.startswith("stemma:src."):
    ...
    elif artifact_id.startswith("conn.") or artifact_id.startswith("stemma:conn."):
        # Support both new format (conn.000001) and legacy (stemma:conn.*)
        conn_filename = artifact_id.replace("stemma:conn.", "conn.")
