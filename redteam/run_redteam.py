...
def test_paper_1_hubble_tension():
    ...
    hubble_quantity = next((e for e in entities.values() if e.get("id") == "stemma:phys.hubble-constant"), None)
    if hubble_quantity:
...
def test_evidence_ledger():
    ...
    # Test lineage query
    result = subprocess.run([
        sys.executable, "scripts/evidence_ledger.py", "--lineage", "stemma:phys.newtons-second-law"
    ], capture_output=True, text=True, cwd=ROOT)
